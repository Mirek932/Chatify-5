...
var WaitForSocketToInit = setTimeout(() => {
  if (socket) {
    socket.on(
      'chat message',
      (msg: string, channel: string, author: string, time: string) => {
    ...
    socket.on(
      'single chat message',
      (msg: string, user: string, author: string, time: string) => {
        console.log(user + ' === ' + socket.id);
        if (user === socket.id) NewMessage(msg, author, time);
...
function NewMessage(
  msg: string,
  author: string = 'Anonym',
  time: string = '01.01.2000'
) {
