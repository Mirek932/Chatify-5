...
var channelDisplayName: string =
  params.get('channelDisplayName') || 'INVALIDdISPLAYNAME';
if (
  channelID == 'EMPTYhEADERNOPE' ||
  channelDisplayName == 'INVALIDdISPLAYNAME'
)
...
function Str_Random(length: number) {
  let result = '';
  const characters =
    'abcdeghijklmnopqrstuvwxyz0123456789ABCDEGHIJKLMNOPQRSTUVWXYZ-';
