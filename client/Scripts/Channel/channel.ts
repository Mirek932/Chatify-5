...
var waitForSocket = setTimeout(() => {
  if (socket) {
    socket.on(
      'create chat room',
      (channelID: string, channelDisplayName: string, socketID: string) => {
        if (socketID == 'public') AppendChatRoom(channelDisplayName, channelID);
        else if (socketID == staticID)
          AppendChatRoom(channelDisplayName, channelID, true);
      }
    );
    socket.on(
      'single create chat room',
      (
        clientID: string,
        channelID: string,
...
const channelDisplayName = <HTMLInputElement>(
  document.getElementById('room-name')
);
const CopyLinkButton = <HTMLButtonElement>(
  document.getElementById('copy-room-link')
);
CopyLinkButton.addEventListener('click', () => {
  navigator.clipboard.writeText(
    window.location.origin +
      `/invite?channelID=${currentChannel}&channelDisplayName=${currentChannelName}`
  );
});
...
function AppendChatRoom(
  ...
  Delete.innerHTML = '🗑️';
  Delete.style =
    'background-color:rgba(226, 226, 226, 0); border-radius: 15px; margin-left: auto;';
  ...
  if (important) {
    ...
    Delete.addEventListener('click', () => {
      ...
      socket.emit(
        'delete chat room',
        RoomParent.getAttribute('channelID'),
        staticID
      );
    });
    ...
    if (nameElement) {
      if (
        !nameElement.innerHTML
          .toLowerCase()
          .includes(RoomSearch.value.toLocaleLowerCase())
      )
