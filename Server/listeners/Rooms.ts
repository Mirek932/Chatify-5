...
export default function registerRoomHandlers(socket: Socket, io: Server) {
  ...
  socket.on(
    ...
    (
      ...
      GivenStaticID: string = 'FFFFFFFFF'
    ) => {
      console.log(
        `Chat room ${channelDisplayName} created with id ${channelID}`
      );
      var StaticID = 'public';
      if (!isPublic) StaticID = GivenStaticID;
  ...
  socket.on('delete chat room', (channelID: string, staticID: string) => {
    ...
    const index = Rooms.findIndex(
      (room) => room.RoomID === channelID && room.StaticID === staticID
    );
...
export function JoinRoom(socketID: string, channel: string) {
  ...
  if (!oldCM.includes(socketID)) channelMembers.set(channel, oldCM + socketID);
}
