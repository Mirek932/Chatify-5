...
export default function registerChatHandlers(socket: Socket, io: Server) {
  ...
  socket.on('chat message', (msg: string, author: string, channel: string) => {
    ...
    if (SocketMPM) UsersMPM.set(socket.id, SocketMPM + 1);
    if (SocketMPM < MPM_LIMIT) {
      ...
      socket.emit(
        'single chat message',
        'Du hast dein Limit von Nachrichten erreicht! Warte eine Minute...',
        socket.id
      );
      socket.emit(
        'info box',
  ...
  socket.on('set CACC', (channelID: string, isTyping: boolean) => {
    ...
    if (isTyping && !CACCs.includes(channelID)) CACCs.push(channelID);
    else if (index !== -1) CACCs.slice(index, 1);
  });

  socket.on(
    'delete message',
    (msg: string, room: string, author: string, time: string) => {
...
setInterval(() => {
  // Reset the MPMs for each user
  UsersMPM.forEach((value, key: string) => UsersMPM.set(key, 0));
}, 1000 * 60);

export async function ClientLoadMessages(
  clientID: string,
  chatRoom: string,
  io: Server
) {
